import re
from getbook.core import Parser, Book, Section

BOOK_PATTERN = re.compile(r'/read/(\d+)/(\d+)/$')
PAGE_PATTERN = re.compile(r'/read/\d+/\d+/\d+\.html$')


class WudiunParser(Parser):
    NAME = 'wudiun'
    ALLOWED_DOMAINS = ['www.wudiun.net']
    ...
    def parse(self):
        ...
        return super(WudiunParser, self).parse()
    ...
    def parse_book(self, book_id):
        ...
        el = self.dom.find('h1')
        title = el.get_text()
        title = title.replace(u'最新章节更新列表', '').strip()
